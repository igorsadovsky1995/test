$(document).ready(function(){
    ...
    (function () {
        ...
        if (Hls.isSupported()) {
          var hls = new Hls();
          hls.loadSource('https://hls-01-radio53.hostingradio.ru/radio53/playlist.m3u8');
    ...
    function volume(){
        var music = $('#music');
        music[0].volume=$('.controls-site-sound__range').val() / 100;
    }
    function player(){
        $('.plyr').hide();
    ...
    $('.controls-site-sound__range').change(volume);
    ...
    $('.controls-site-sound__volume-svg').click(function(){
        ...
        if(!$('.controls-site-sound__volume-svg').hasClass('quiet')){
            ...
            memoryVolume=$('.controls-site-sound__range').val() / 100;
            music[0].volume=0;
            console.log(0)
        }else{
